...
contract FXEngine is ReentrancyGuard, Pausable, Ownable {
        ...
        returns (uint256 amountOut)
    {
        ...
        // ── Compute output amount + total dynamic fee
        uint256 fee;
        (amountOut, fee) = _computeAmountOut(amountIn, poolIn, poolOut);
        require(amountOut >= minAmountOut, "FXEngine: slippage exceeded");

        // ── Platform fee: pool-level treasury share (e.g. 30% of fee → platformTreasury)
        uint256 platformFee;
        if (poolOut.platformTreasury() != address(0) && poolOut.platformFeeBps() > 0) {
            platformFee = (fee * poolOut.platformFeeBps()) / FEE_DENOMINATOR;
        }

        // ── Protocol fee: engine-level yield-distributor share (fraction of remaining fee)
        uint256 pFee;
        if (distributor != address(0) && protocolFeeRate > 0) {
            pFee = (fee * protocolFeeRate) / FEE_DENOMINATOR;
        }

        uint256 totalRelease = amountOut + platformFee + pFee;
        ...
        if (platformFee > 0) {
            ...
            emit ProtocolFeeCollected(tokenOut, platformFee);
        }
        ...
        returns (uint256 amountOut, uint256 fee)
    {
        ...
        // Deduct the dynamic fee; LP portion stays in pool, platform + protocol fees released separately
        uint256 effectiveFeeRate = poolOut.getEffectiveFeeRate(grossOut);
        fee = (grossOut * effectiveFeeRate) / FEE_DENOMINATOR;
