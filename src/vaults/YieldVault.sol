...
contract YieldVault is ERC4626, IFXPool, ReentrancyGuard, Pausable, Ownable {
    ...
    /// @notice YieldVault uses a fixed harvest fee; no utilization-based scaling.
    function getEffectiveFeeRate(uint256) external view override returns (uint256) {
        return feeRate;
    }

    /// @notice No dynamic fee cap — vault fee is always fixed.
    function maxDynamicFeeRate() external view override returns (uint256) {
        return feeRate;
    }

    /// @notice YieldVault does not distribute a separate platform fee.
    function platformFeeBps() external pure override returns (uint256) {
        return 0;
    }

    /// @notice No platform treasury for YieldVault.
    function platformTreasury() external pure override returns (address) {
        return address(0);
    }

    /// @notice Not applicable — platformFeeBps is 0 so this is never called by FXEngine.
    function releasePlatformFee(uint256) external pure override {
        revert("YieldVault: no platform fee");
    }
